...
def mass_move_channel(channel_name, target_playlist_name):
    ...
    to_move = []
    for p in playlists:
        # Avoid moving if target and source are the same
    ...
    if not to_move:
        print(f"No videos from '{channel_name}' found outside of '{target_playlist_name}'.")
        return
        
    print(f"Found {len(to_move)} videos to move to '{target_playlist_name}'.")
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python mass_move.py <channel_name> <target_playlist_name>")
    else:
        mass_move_channel(sys.argv[1], sys.argv[2])
