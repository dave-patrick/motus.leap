...
(function () {
  ...
  const PROV_HINTS = {
    ...
    z_ai: 'Get your API key at api.z.ai — access GLM-5.2 and other GLM models.',
  ...
  function updateProvType(keepCurrentValues) {
    ...
    const namePh = { openai: 'My OpenAI', anthropic: 'My Claude', groq: 'My Groq', grok: 'My Grok', google: 'My Gemini', openrouter: 'My OpenRouter', z_ai: 'My z.ai', custom: 'My LLM' };
  ...
  async function loadExcludedPlaylists() {
    const listEl = $('#excluded-playlists-list');
    if (!listEl) return;
    try {
      const [exRes, plRes] = await Promise.all([
        api('/api/playlists/excluded').catch(() => ({ excluded_playlists: [] })),
        api('/api/playlists').catch(() => ({ playlists: [] })),
      ]);
      excludedSet = new Set(exRes.excluded_playlists || []);
      const pls = plRes.playlists || plRes.items || (Array.isArray(plRes) ? plRes : []);
      allPlaylists = pls;
      renderExcludedPlaylistsList();
    } catch (e) {
      if (listEl) listEl.innerHTML = '<div class="text-xs text-[#dc2626] p-2">' + sanitize(e.message) + '</div>';
    }
  }

  function renderExcludedPlaylistsList() {
    const listEl = $('#excluded-playlists-list');
    const badgeEl = $('#excluded-count-badge');
    const searchInput = $('#excluded-search');
    const query = (searchInput ? searchInput.value : '').toLowerCase().trim();
    if (!listEl) return;
    if (badgeEl) badgeEl.textContent = excludedSet.size + ' excluded';

    const filtered = allPlaylists.filter(p => {
      const title = (p.title || p.name || p.id || '').toLowerCase();
      return !query || title.includes(query);
    });

    if (!filtered.length) {
      listEl.innerHTML = '<div class="text-xs text-gray-500 p-2 sm:col-span-2">No matching playlists found.</div>';
      return;
    }

    listEl.innerHTML = filtered.map(p => {
      const pid = p.id;
      const title = esc(p.title || p.name || pid);
      const isExcluded = excludedSet.has(pid);
      return '<label class="flex items-center justify-between p-2.5 rounded-lg bg-[#20242c] border border-[#2a2f3a] hover:border-[#3a404d] cursor-pointer text-xs transition-colors">' +
        '<div class="flex items-center gap-2.5 min-w-0 pr-2">' +
        '<input type="checkbox" class="excluded-toggle accent-[#eab308]" data-pid="' + esc(pid) + '"' + (isExcluded ? ' checked' : '') + '>' +
        '<span class="text-gray-200 font-medium truncate">' + title + '</span>' +
        '</div>' +
        (isExcluded ? '<span class="text-[9px] bg-[#eab308]/15 text-[#eab308] border border-[#eab308]/30 px-2 py-0.5 rounded font-mono shrink-0">Excluded</span>' : '') +
        '</label>';
    }).join('');

    $all('.excluded-toggle').forEach(chk => {
      chk.addEventListener('change', async () => {
        const pid = chk.getAttribute('data-pid');
        try {
          const res = await api('/api/playlists/excluded/toggle', {
            method: 'POST',
            body: JSON.stringify({ playlist_id: pid })
          });
          excludedSet = new Set(res.excluded_playlists || []);
          renderExcludedPlaylistsList();
          toast(res.status === 'excluded' ? 'Playlist excluded from mapping/AI' : 'Playlist included in mapping/AI', 'success');
        } catch (e) {
  ...
  if ($('#excluded-search')) {
    $('#excluded-search').addEventListener('input', renderExcludedPlaylistsList);
  }
