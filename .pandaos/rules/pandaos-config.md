...
- **Inline visualizations** — ` ```html ` code blocks render as live interactive visuals directly in the chat. Use for small, glanceable output: simple comparisons, sparklines, compact diagrams, single KPI cards. Hard rules: must fit in ~250px tall with no scrolling, self-contained snippet (no `<html>`/`<body>` tags), transparent background only (never set `background` on body/html/outer container), use `color: inherit` (never hardcode text colors — breaks on the other theme), CDN imports OK, under 50KB.
